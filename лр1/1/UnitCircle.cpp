...
void __fastcall TFormCircle::Timer1Timer(TObject *Sender)
{
	...
	if (x-r==Shape1->Left+Shape1->Width+10) {
		...
		Button1Click(Sender);
		ShowMessage("Невозможно переместить фигуру, используя введенные данные, т.к. при перемещении фигура выходит за рамку\nВведите новые данные для выбранной операции.");
	}
	if (p==true) {
		Timer1->Enabled=false;
		Timer2->Enabled=true;
...
{	if (Edit11->Text=="") Edit11->Text="1,0";}
//---------------------------------------------------------------------------
void __fastcall TFormCircle::Button7Click(TObject *Sender)
{
	Circ ob1(px,py,pr);
	ob1.changing(StrToFloat(Edit11->Text));
}
//---------------------------------------------------------------------------
...
void __fastcall TFormCircle::Image1Click(TObject *Sender)
{
	Edit11->Text="1,02";
	Timer3->Enabled=true;
}
...
void __fastcall TFormCircle::Image2Click(TObject *Sender)
{
	Edit11->Text="0,98";
	Timer3->Enabled=true;
}
...
void __fastcall TFormCircle::FormClick(TObject *Sender)
{
	*px=500+10+FormCircle->Shape1->Left+FormCircle->Shape1->Width;
	*py=500+10;
	*pr=100;
	Circ ob1(px,py,pr);
	draw();
