...
int Mnogoug::getDegree()					{ return degree;}
void Mnogoug::setDegree(int d)				{ degree=d;}
...
void Mnogoug::turningVersh(){
	int i;
	weightCenterX+=FormMnogoug->Shape1->Left+FormMnogoug->Shape1->Width+10;
	weightCenterY+=10;
	for (i = 0; i < 10; i++) {
		if (xkoor[i]>0 && ykoor[i]>0) {
			int dx,dy;
			float rast=sqrt(powl(xkoor[i]-weightCenterX,2)+powl(ykoor[i]-weightCenterY,2));
			if (weightCenterX<=xkoor[i] && weightCenterY<=ykoor[i]){
				dy=ykoor[i]-weightCenterY;
				float a1;
				if (dy/rast<=1) a1=asin(dy/rast);
				else a1=M_PI/2;
				float a2=angle-a1;
				xkoor[i]=weightCenterX+cos(a2)*rast;
				ykoor[i]=weightCenterY-sin(a2)*rast;
			}
			if (weightCenterX<=xkoor[i] && weightCenterY>=ykoor[i]) {
				dy=abs(ykoor[i]-weightCenterY);
				float a1;
				if (dy/rast<=1) a1=asin(dy/rast);
				else a1=M_PI/2;
				float a2=angle+a1;
