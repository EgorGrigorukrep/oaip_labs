...
void Tree::deleteItem(TreeItem* item,TreeItem* parent){
	if (item!=parent) {
		if (item->right==NULL && item->left==NULL) {
			...
			TreeItem* rightPointer;
			while (temp->left!=NULL){
				temp=temp->left;
			}
			rightPointer=temp->right;
			temp->left=item->left;
...
int newTree::maxLevelsAmount(TreeItem* tempRoot,int level){
	if (tempRoot==NULL) {
		return level-1;
	}
	else{
		int leftDepth,rightDepth;
		leftDepth=maxLevelsAmount(tempRoot->left,level+1);
		rightDepth=maxLevelsAmount(tempRoot->right,level+1);
...
void newTree::treePassing(int currentLevel,int requiredLevel, TreeItem* tempRoot,int* amount){
	if (currentLevel==requiredLevel) {
		...
		if (tempRoot->right!=NULL) treePassing(currentLevel+1,requiredLevel,tempRoot->right,amount);
		if (tempRoot->left!=NULL) treePassing(currentLevel+1,requiredLevel,tempRoot->left,amount);
	}
}

int newTree::findLeafsAmount(int level, Tree tree){
	int amount=0;
	treePassing(1,level,tree.getRoot(),&amount);
	return amount;
}
