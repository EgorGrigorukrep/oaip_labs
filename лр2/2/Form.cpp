...
AnsiString SaveAddress;
bool hasDialogAppeared=false;
int counter=0;
...
void __fastcall TForm1::FormActivate(TObject *Sender)
{
	...
	delete f;
	if (amount<=10) StringGrid1->Height=(amount+1)*(StringGrid1->DefaultRowHeight+1);
	else {
		StringGrid1->Height=(11)*(StringGrid1->DefaultRowHeight+1);
    }
	...
	Button15->Left=Button9->Left;
	Button15->Top=Button9->Top+Button9->Height+30;
...
void __fastcall TForm1::Button6Click(TObject *Sender)
{
	...
	if (RadioButton1->Checked==true) ShowMessage("На следующие сутки необходимо "+IntToStr(fuelAmount)+" литров топлива");
	else RadioButton1->Caption="Hеобходимо "+IntToStr(fuelAmount)+" литров топлива";
	delete []array;
}
//---------------------------------------------------------------------------
...
{
	if (Edit1->Tag == 5 || Edit1->Tag == 6)counter++;
	if (counter==2) {
		if (Edit1->Tag == 5 || Edit1->Tag == 6) Edit2->SetFocus();
		counter=0;
	}
	if (Key==13) {
		bool isRight;
...
void __fastcall TForm1::Button15Click(TObject *Sender)
{
	if (SaveDialog2->Execute()) {
		int i,j;
		Variant ExcelApp,ExcelBooks,Book,BookSheets,Sheet,Cell;
		...
		BookSheets=Book.OlePropertyGet("Worksheets");
		AnsiString title="Расписание полетов на ";
		TDateTime Date;
		...
		title+=date;
		Sheet=BookSheets.OlePropertyGet("Item",1);
		Sheet.OlePropertySet("Name",StringToOleStr(title));
		Cell=Sheet.OlePropertyGet("Range",(WideString)"A1:F1");
		...
		Cell.OlePropertyGet("Interior").OlePropertySet("ColorIndex",35);
		Cell.OlePropertyGet("Font").OlePropertySet("Size",20);
		Cell.OlePropertySet("Value",StringToOleStr(title));
		for (i = 0; i < StringGrid1->ColCount; i++) {
			for (j=0; j < StringGrid1->RowCount+2; j++) {
				Cell=Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",j+1,i+1);
				Cell.OlePropertySet("RowHeight",20);
				Cell.OlePropertySet("ColumnWidth",30);
			}
		}
		for (i = 0; i < StringGrid1->ColCount; i++) {
			for (j=0; j < StringGrid1->RowCount; j++) {
				Cell=Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",j+3,i+1);    //j V  i >
				Cell.OlePropertySet("HorizontalAlignment",-4108);
				Cell.OlePropertySet("VerticalAlignment",-4108);
				Cell.OlePropertyGet("Font").OlePropertySet("Size",18);
				Cell.OlePropertySet("Value",StringToOleStr(StringGrid1->Cells[i][j]));
			}
		}
		AnsiString CellName1,CellName2,UnitedStrings;
		CellName1="A"+IntToStr(StringGrid1->RowCount+4);
		CellName2="F"+IntToStr(StringGrid1->RowCount+4);
		UnitedStrings=CellName1+":"+CellName2;
		Cell=Sheet.OlePropertyGet("Range",StringToOleStr(UnitedStrings));
		Cell.OleProcedure("Merge");
		Cell=Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",StringGrid1->RowCount+4,1);
		Cell.OlePropertySet("HorizontalAlignment",-4108);
		Cell.OlePropertySet("VerticalAlignment",-4108);
		Cell.OlePropertyGet("Interior").OlePropertySet("ColorIndex",35);
		Cell.OlePropertyGet("Font").OlePropertySet("Size",20);
		...
		Cell.OlePropertySet("Value",StringToOleStr(RadioButton1->Caption));
		CellName1="A"+IntToStr(StringGrid1->RowCount+6);
		CellName2="C"+IntToStr(StringGrid1->RowCount+6);
		UnitedStrings=CellName1+":"+CellName2;
		Cell=Sheet.OlePropertyGet("Range",StringToOleStr(UnitedStrings));
		Cell.OleProcedure("Merge");
		Cell=Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",StringGrid1->RowCount+6,1);
		Cell.OlePropertyGet("Font").OlePropertySet("Size",16);
		Cell.OlePropertySet("HorizontalAlignment",-4108);
		Cell.OlePropertyGet("Font").OlePropertySet("Underline",4);
		Cell.OlePropertySet("Value",(WideString)"Диспетчер технической службы аэропорта");
		Cell=Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",StringGrid1->RowCount+6,5);
		Cell.OlePropertyGet("Font").OlePropertySet("Size",16);
		Cell.OlePropertySet("HorizontalAlignment",-4108);
		Cell.OlePropertyGet("Font").OlePropertySet("Italic",true);
		Cell.OlePropertyGet("Font").OlePropertySet("Underline",4);
		Cell.OlePropertySet("Value",(WideString)"Иванов И.И.");
		Cell=Sheet.OlePropertyGet("Cells").OlePropertyGet("Item",StringGrid1->RowCount+6,4);
		Cell.OlePropertyGet("Font").OlePropertySet("Size",18);
		Cell.OlePropertySet("HorizontalAlignment",-4108);
		Cell.OlePropertyGet("Font").OlePropertySet("Name",(WideString)"Vladimir Script");
		Cell.OlePropertySet("Value",(WideString)"Ivanov");
		ExcelApp.OlePropertyGet("Workbooks").OlePropertyGet("Item",1).OleProcedure("SaveAs",StringToOleStr(SaveDialog2->FileName+".xlsx"));
